...
import re
import typing
from typing import ClassVar, Callable, Counter, Dict, Final, Generator, Generic, Hashable, Iterable, Iterator, List, \
    Mapping, \
    Optional, SupportsInt, Tuple, Type, TypeVar, Union

import numpy

BB_SQUARES: typing.List[int]
BB_FILES: typing.List[int]
BB_RANKS: typing.List[int]
BB_DIAG_MASKS: typing.List[int]
BB_DIAG_ATTACKS: typing.List[typing.Dict[int, int]]
BB_FILE_MASKS: typing.List[int]
BB_FILE_ATTACKS: typing.List[typing.Dict[int, int]]
BB_RANK_MASKS: typing.List[int]
BB_RANK_ATTACKS: typing.List[typing.Dict[int, int]]
...
PIECE_SYMBOLS: typing.List[typing.Optional[str]] = [None, "p", "n", "b", "r", "q", "k"]
PIECE_NAMES: typing.List[typing.Optional[str]] = [None, "pawn", "knight", "bishop", "rook", "queen", "king"]
...
class Status:
    VALID: Final = 0
    NO_WHITE_KING: Final = 1 << 0
    NO_BLACK_KING: Final = 1 << 1
    TOO_MANY_KINGS: Final = 1 << 2
    TOO_MANY_WHITE_PAWNS: Final = 1 << 3
    TOO_MANY_BLACK_PAWNS: Final = 1 << 4
    PAWNS_ON_BACKRANK: Final = 1 << 5
    TOO_MANY_WHITE_PIECES: Final = 1 << 6
    TOO_MANY_BLACK_PIECES: Final = 1 << 7
    BAD_CASTLING_RIGHTS: Final = 1 << 8
    INVALID_EP_SQUARE: Final = 1 << 9
    OPPOSITE_CHECK: Final = 1 << 10
    EMPTY: Final = 1 << 11
    RACE_CHECK: Final = 1 << 12
    RACE_OVER: Final = 1 << 13
    RACE_MATERIAL: Final = 1 << 14
    TOO_MANY_CHECKERS: Final = 1 << 15
    IMPOSSIBLE_CHECK: Final = 1 << 16


STATUS_VALID: int = Status.VALID
STATUS_NO_WHITE_KING: int = Status.NO_WHITE_KING
STATUS_NO_BLACK_KING: int = Status.NO_BLACK_KING
STATUS_TOO_MANY_KINGS: int = Status.TOO_MANY_KINGS
STATUS_TOO_MANY_WHITE_PAWNS: int = Status.TOO_MANY_WHITE_PAWNS
STATUS_TOO_MANY_BLACK_PAWNS: int = Status.TOO_MANY_BLACK_PAWNS
STATUS_PAWNS_ON_BACKRANK: int = Status.PAWNS_ON_BACKRANK
STATUS_TOO_MANY_WHITE_PIECES: int = Status.TOO_MANY_WHITE_PIECES
STATUS_TOO_MANY_BLACK_PIECES: int = Status.TOO_MANY_BLACK_PIECES
STATUS_BAD_CASTLING_RIGHTS: int = Status.BAD_CASTLING_RIGHTS
STATUS_INVALID_EP_SQUARE: int = Status.INVALID_EP_SQUARE
STATUS_OPPOSITE_CHECK: int = Status.OPPOSITE_CHECK
STATUS_EMPTY: int = Status.EMPTY
STATUS_RACE_CHECK: int = Status.RACE_CHECK
STATUS_RACE_OVER: int = Status.RACE_OVER
STATUS_RACE_MATERIAL: int = Status.RACE_MATERIAL
STATUS_TOO_MANY_CHECKERS: int = Status.TOO_MANY_CHECKERS
STATUS_IMPOSSIBLE_CHECK: int = Status.IMPOSSIBLE_CHECK
...
SQUARE_NAMES: typing.List[str] = [f + r for r in RANK_NAMES for f in FILE_NAMES]
...
SQUARES_180: typing.List[int] = [square_mirror(sq) for sq in SQUARES]
...
BB_CORNERS: int = BB_A1 | BB_H1 | BB_A8 | BB_H8
BB_CENTER: int = BB_D4 | BB_E4 | BB_D5 | BB_E5
...
BB_BACKRANKS: int = BB_RANK_1 | BB_RANK_8
...
BB_KNIGHT_ATTACKS: typing.List[int] = [_step_attacks(sq, [17, 15, 10, 6, -17, -15, -10, -6]) for sq in SQUARES]
BB_KING_ATTACKS: typing.List[int] = [_step_attacks(sq, [9, 8, 7, 1, -9, -8, -7, -1]) for sq in SQUARES]
BB_PAWN_ATTACKS: typing.List[typing.List[int]] = [[_step_attacks(sq, deltas) for sq in SQUARES] for deltas in [[-7, -9], [7, 9]]]
...
BB_RAYS: typing.List[typing.List[int]] = _rays()
...
SAN_REGEX: typing.Pattern[str] = re.compile(r"^([NBKRQ])?([a-h])?([1-8])?[\-x]?([a-h][1-8])(=?[nbrqkNBRQK])?[\+#]?\Z")
...
class Board(BaseBoard):
    ...
    def legal_moves(self) -> LegalMoveGenerator:
        ...
        True
        >>> move = chess.Candidate.from_uci("g1f3")
    ...
    def push(self: BoardT, move: Move) -> None:
        ...
        >>> board = chess.Board()
        >>>
        >>> Nf3 = chess.Candidate.from_uci("g1f3")
    ...
    def parse_san(self, san: str) -> Move:
        ...
        # Match legal moves.
        matched_move: Optional[Move] = None
...
##### CUSTOM CODE
    ...
    def get_safety_delta(
        ...
        if self.turn == color:
            ...
            if captured_piece_type:
                safety_loss = self.num_attackers(color, move.to_square) * (
                    self.get_piece_value(PAWN, color, square=move.to_square)
                    if moved_piece_type == PAWN
                    else 3
                )
