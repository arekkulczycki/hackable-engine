...
class GCN(nn.Module):
    def __init__(
        ...
        ), "None action_space input: action_space should be assigned"
    ...
    def forward(self, x, edge_index):
        ...
        x = global_add_pool(
            self.layer_norm(x), torch.LongTensor([0 for _ in range(4)]).to(device)
        )
...
if __name__ == "__main__":
    ...
    for epi in range(episodes):
        ...
        if epi % print_interval == 0 and epi != 0:
            print(
                "# of episode :{}, avg score : {}".format(
                    epi, sum(score_list[-print_interval:]) / print_interval
                )
            )
