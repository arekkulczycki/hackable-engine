...
class GraphGATGIN(BaseModule):
    ...
    def setup_graph_feature_extractor(self):
        ...
        prev_heads = 1
        for i, (channels, heads) in enumerate(zip(self.gnn_shape, self.gnn_heads)):
            if i < 2:
                gnn.append(
                    GATv2Conv(prev * prev_heads, channels, heads=heads, concat=True, edge_dim=3, add_self_loops=False)
                )
            else:
                mlp_input = prev * prev_heads if i <= 2 else prev
                mlp = nn.Sequential(
                    nn.Linear(mlp_input, mlp_input, device=self.device),
                    LayerNorm(mlp_input),
                    nn.ReLU(),
                    nn.Dropout(self.dropouts),
                    nn.Linear(mlp_input, channels, device=self.device),
                )
                gnn.append(GINConv(mlp, train_eps=True))

            res_input_channels = prev * prev_heads if i <= 2 else prev
            res_output_channels = channels * heads if i <= 2 else channels
            if res_input_channels == res_output_channels:
                residuals.append(nn.Identity(device=self.device))
            else:
                residuals.append(nn.Linear(res_input_channels, res_output_channels, device=self.device))
            norms.append(LayerNorm(res_output_channels) if i <= 2 else nn.Identity(device=self.device))
            prev = channels
            prev_heads = heads

        self.gnn: nn.ModuleList[GATv2Conv, GINConv] = nn.ModuleList(gnn)
        self.residuals = nn.ModuleList(residuals)
        self.norms: nn.ModuleList[LayerNorm | nn.Identity] = nn.ModuleList(norms)
