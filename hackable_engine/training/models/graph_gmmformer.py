...
from torch_geometric.nn import GMMConv, TransformerConv
...
class GraphGMMformer(BaseModule):
    ...
    def __init__(
        ...
        self.gnn_heads = gnn_heads if isinstance(gnn_heads, int) else gnn_heads[0]
        self.mlp_shape = mlp_shape
        self.device = device
        self.gmm_layers = gmm_layers
    ...
    def setup_graph_feature_extractor(self):
        ...
        for i, channels in enumerate(self.gnn_shape):
            if i <= self.gmm_layers:
                gnn.append(
                    GMMConv(prev, channels, dim=2, kernel_size=self.kernel_size)
                )
            else:
                gnn.append(TransformerConv(prev, channels, heads=self.gnn_heads, concat=False, edge_dim=3))
