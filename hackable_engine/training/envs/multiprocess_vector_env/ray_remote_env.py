...
from ray.rllib.env.base_env import ASYNC_RESET_RETURN
from ray.rllib.utils.annotations import override, PublicAPI
from ray.rllib.utils.typing import EnvType, EnvID, MultiAgentDict
...
class RayRemoteEnv(BaseEnv):
    ...
    def __init__(
        ...
        self.local_env = make_env(0, env_per_worker, color)
        ...
        self.buf_obs = np.zeros(self.num_envs, dtype=np.float32)
        # OrderedDict(
        #     [
    ...
    def _save_obs(self, env_idx: int, obs: VecEnvObs) -> None:
        for key in self.keys:
            if key is None:
                self.buf_obs[key][
                    env_idx * self.env_per_worker : (env_idx + 1) * self.env_per_worker
                ] = obs[0]
            else:
                self.buf_obs[key][
                    env_idx * self.env_per_worker : (env_idx + 1) * self.env_per_worker
    ...
    def poll(
        ...
        ObsType, ArrayType, ArrayType, ArrayType, dict[str, Any]
    ]:
        if self.actors is None:

            def make_remote_env(i):
                logger.info("Launching env {} in remote actor".format(i))
                return _RemoteSingleAgentEnv.remote(
                    self.make_local_env, i, self.env_per_worker, self.color
                )
        ...
        for obj_ref in ready:
            ...
            ob, rew, done, _, info = ray.get(obj_ref)

            if "episode" in info:
                if isinstance(done, bool):
                    # self.time_queue.extend(info["episode"]["t"])
                    self.length_queue.extend(info["episode"]["l"])
                    self.return_queue.extend(info["episode"]["r"])
                else:
                    episode = info["episode"]
                    for i in np.where(done):
                        # self.time_queue.extend(episode["t"][i])
                        self.return_queue.extend(episode["r"][i])
                        self.length_queue.extend(episode["l"][i])

            # self._save_obs(env_id, ob)
            self.buf_obs[
                env_id * self.env_per_worker: (env_id + 1) * self.env_per_worker
            ] = ob[0]
            self.buf_rews[
                env_id * self.env_per_worker : (env_id + 1) * self.env_per_worker
            ] = rew
        ...
        return (
            ...
            np.copy(self.buf_blank),
            deepcopy(self.buf_infos),
        )

    def _obs_from_buf(self) -> VecEnvObs:
        return dict_to_obs(
            self.local_env.observation_space, copy_obs_dict(self.buf_obs)
        )
    ...
    def send_actions(self, action_list) -> None:
        for worker_id in range(self.num_workers):
            actions = action_list[
                worker_id * self.env_per_worker : (worker_id + 1) * self.env_per_worker
            ]
...
class _RemoteSingleAgentEnv:
    ...
    def __init__(self, make_env, i, env_per_worker, color):
        ...
        self.env: VectorEnv = make_env(i, env_per_worker, color, [None])

    def reset(self):
        return self.env.reset(), 0, False, False, {}
