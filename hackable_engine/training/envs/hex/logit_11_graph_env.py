...
class Logit11GraphEnv(BaseEnv):
    ...
    def _make_opponent_move(self, n_moves):
        minimum_logical_moves = 0.94
        len_results = len(self.results)
        win_percentage = (
            sum(self.results) / len_results
            if len_results >= 5
            else minimum_logical_moves
        )
        ...
        if (
            ...
            and choices(
                (True, False), weights=(random_move_weight, 1 - random_move_weight)
            )[0]
        ):
            move = self._make_random_move()
            self.opponent_move_random = True
        else:
            if self.opp_ort_session:
                move = self._make_trained_opponent_move(n_moves + 1)
            else:
                move = self._make_logical_move(n_moves + 1)
            self.opponent_move_random = False
        self.opponent_move = move

    def _make_trained_opponent_move(self, n_moves):
        """The action chosen is the highest value regardless of color, using `argmax`."""

        logits = self.opp_ort_session.run(None, {"inputs": np.expand_dims(self.observation_from_board(), axis=0)})

        move = Move.from_c(np.argmax(logits).item(), size=self.BOARD_SIZE)
        try:
            self.board.push(move)
        except ValueError:
            # don't add +1 to n_moves, it's expected to be added before calling this function already
            move = self._make_logical_move(n_moves)
    ...
    def _make_logical_move(self, n_moves: int, color: bool | None = None) -> Move:
        ...
        f = (
            self._get_distance_score
            if n_moves < 3 * self.BOARD_SIZE
            else self._get_distance_score_perf
        )
        for move in (
            sample(legal_moves, subset_size)
            if subset_size < len(legal_moves)
            else legal_moves
        ):
            self.board.push(move)
            try:
